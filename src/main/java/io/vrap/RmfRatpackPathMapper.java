...
public class RmfRatpackPathMapper {
    private static final String DIRECTORY_PATTERN = "[-a-zA-Z0-9@:%_\\+.~#?&=]+";

    public static String map(String pathTemplate) {
        return RmfRatpackPathMapper.map(pathTemplate, Lists.emptyList());
    }

    public static String map(String pathTemplate, List<UriParameter> uriParameters) {
    ...
    private static String subPath(String pathTemplate, List<UriParameter> uriParameters) {
        if (!pathTemplate.contains("{") && !pathTemplate.contains("}")) {
            return pathTemplate;
        }
        ...
        for (final String match : allMatches) {
            ...
            for (final UriParameter uriParamDeclaration : uriParameters) {
                ...
                if (uriParamDeclaration instanceof StringTypeDeclaration) {
                    ...
                    if (uriParamDeclaration.getName().equals(match) && uriPattern != null) {
